 ...
 * AssessmentResult (plus any CV recommendations already in state) directly since
 * a standalone assessment has no screeningId to look up by — see
 * app/api/send-assessment-report's own docs. The roadmap/resource content is
 * recomputed server-side from the same lib/roadmap engine the results page uses. */
export function EmailAssessmentResultForm({
  result,
  role,
  recommendations,
}: {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/send-assessment-report", {
        ...
        body: JSON.stringify({ email, role, result, recommendations }),
      });
