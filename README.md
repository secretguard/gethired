...
- Rendered in the web report via `RoadmapView` once the practical assessment completes, with a toggle between two views of the exact same `generateRoadmap()` output: a **Diagram** view (default) built with `react-organizational-chart` — one small node/branch tree per stage (stage card branching into its action items), stacked and connected vertically — and a **List** view, the original numbered/connected step list, kept as a simpler fallback. `react-organizational-chart` is loaded client-only via `next/dynamic({ ssr: false })` since it touches `document` at module scope, which breaks Next's prerender pass otherwise. Each stage gets its own independent mini-tree rather than one nested tree for the whole roadmap — chaining every stage as a nested child of the previous one made total diagram width balloon additively across all stages (2500px+ for a 4-step roadmap in testing) instead of staying bounded to one stage's action count.
